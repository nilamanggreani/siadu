...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO pengajuan (nama_pengaju, tanggal_ajuan, jenis_ajuan, surat_permohonan, dokumen_iklan, deskripsi_iklan) 
            VALUES ('$nama_pengaju', '$tanggal_ajuan',  'videotron', '$surat_permohonan_data', '$dokumen_iklan_data', '$deskripsi_iklan')";

    $stmt = mysqli_prepare($conn, $sql);

    if (mysqli_stmt_execute($stmt)) {
        // Jika query berhasil, tampilkan pesan sukses
        echo "<script>alert('Permohonan penayangan videotron berhasil dibuat.'); 
        window.location.href = 'ajuan-videotron-saya.php';</script>"; // arahkan ke cek status ajuan
    } 
